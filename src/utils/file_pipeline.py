...
# =========================================================
class TextExtractor:
    """확장자별 문서와 이미지 데이터 추출 및 예외 처리 클래스"""
...
class FileAnalyzer:
    ...
    def _get_file_info(self, file_path: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "content_hints": self._collect_content_hints(file_path),
        }

    @staticmethod
    def _collect_content_hints(file_path: str) -> str:
        """LLM 호출 전에 형식별 메타데이터를 짧고 안전한 힌트로 제공합니다."""
        hints = []
        extension = os.path.splitext(file_path)[1].lower()
        try:
            if extension == ".pdf":
                metadata = PdfReader(file_path).metadata or {}
                for key in ("/Title", "/Subject", "/Author", "/Keywords"):
                    if metadata.get(key):
                        hints.append(f"PDF {key[1:]}: {metadata[key]}")
            elif extension == ".docx":
                props = Document(file_path).core_properties
                for label, value in (("DOCX 제목", props.title), ("DOCX 주제", props.subject), ("DOCX 키워드", props.keywords)):
                    if value:
                        hints.append(f"{label}: {value}")
            elif extension in {".jpg", ".jpeg", ".tif", ".tiff"}:
                with Image.open(file_path) as image:
                    exif = image.getexif()
                    if exif:
                        date = exif.get(36867) or exif.get(306)
                        if date:
                            hints.append(f"EXIF 촬영일: {date}")
            elif extension == ".zip":
                with zipfile.ZipFile(file_path) as archive:
                    names = archive.namelist()[:20]
                    if names:
                        hints.append("ZIP 내부 이름: " + ", ".join(names))
        except Exception:
            pass
        return "\n".join(hints) or "없음"
    ...
    def _normalize_tags(tags: Any, file_info: Dict[str, Any]) -> list[str]:
        ...
        for tag in tags if isinstance(tags, list) else []:
            clean = re.sub(r"\s+", " ", str(tag).strip().lstrip("#"))[:40]
            key = clean.casefold()
            if clean and key not in seen and key not in generic:
