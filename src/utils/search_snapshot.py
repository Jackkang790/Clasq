...
이 모듈은 현재 검색 경로(SQLite LIKE)와 별도로 존재하는 캐시 유틸리티입니다.
Batch 4(로컬 텍스트 인덱스)가 완료된 후 검색 경로를 이 snapshot으로 전환할 수 있습니다.

현재 사용처:
  - FolderAnalysisPlanWorker (Batch 5에서 연결 예정)
  - 직접 호출 시 search_engine.invalidate_snapshot() / refresh_snapshot()
"""
...
def _build_snapshot(db_path: str, generation: int) -> SearchIndexSnapshot:
    ...
    try:
        ...
        # file_fingerprint_cache 테이블이 없을 수 있으므로 안전하게 처리
        try:
        ...
        # file_text_index 테이블은 Batch 4 이후에 채워짐 (없어도 정상 동작)
        try:
