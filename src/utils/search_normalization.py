...
def _particle_agrees_with_stem(stem: str, particle: str) -> bool:
    ...
    # '에/의/에서'는 받침 규칙이 없다. 짧은 명사는 보수적으로 보존한다.
    return len(stem) >= 2


def strip_korean_particle(token: str) -> str:
    """Remove one conservative Korean postposition from a non-trivial token."""
    # Do not mutate extension/English tokens. A Korean morphological analyzer
    # would be needed to safely handle mixed-script words.
    if not token or not all("가" <= character <= "힣" for character in token):
        return token
    for particle in KOREAN_PARTICLES:  # longest suffixes are listed first
