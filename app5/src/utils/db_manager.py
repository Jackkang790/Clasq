...
class FileRegistryManager:
        ...
        try:
            ...
            if dup_row is not None:
                ...
                if self.duplicate_policy == "quarantine":
                    ...
                    # 중복본은 원본 파일과 별도의 검색 레코드를 만들지 않는다.
                    # 실제 이동이 완료된 뒤 DB는 원본 레코드만 유지한다.
                    conn.commit()
                    result["success"] = True
    ...
    def list_files(self) -> List[Dict[str, Any]]:
        """실제 분석 결과를 화면에서 사용할 수 있는 형태로 반환합니다."""
        ...
        try:
            ...
            for row in rows:
                ai_comment = row[6] or ""
                description = ai_comment.split(" / 코멘트: ", 1)[-1] if " / 코멘트: " in ai_comment else ai_comment
                result.append({"id": row[0], "file_name": row[1],
                               "display_name": row[2] or os.path.splitext(row[1])[0],
                               "file_path": row[3], "category": row[4] or "#일반",
                               "tags": row[5] or "", "description": description,
    ...
    def update_file_metadata(self, file_id: int, display_name: str, tags: str, description: str) -> Dict[str, Any]:
        """실제 파일 위치는 건드리지 않고 SavedView의 AI 메타데이터만 갱신합니다."""
        ...
        try:
            ...
            if cursor.rowcount != 1:
                return {"success": False, "message": "저장할 파일 레코드를 찾을 수 없습니다."}
            return {"success": True, "message": "메타데이터를 저장했습니다."}
    ...
    def move_file_safely(self, file_id: int, target_dir: str) -> Dict[str, Any]:
        """파일 이동을 확인한 뒤 DB를 갱신하고, DB 실패 시 이동을 되돌립니다."""
        conn = self._get_conn()
        owns_conn = self._bulk_conn is None
        old_path = ""
        new_path = ""
        try:
            ...
            conn.execute(
                ...
                (os.path.basename(new_path), new_path, target_dir,
                 time.strftime("%Y-%m-%d %H:%M:%S"), file_id),
            )
            conn.commit()
    ...
    def organize_file(self, file_id: int, target_category: str, base_dir: str) -> bool:
        """하위 호환용 단일 파일 정리 API입니다."""
        # 0. 폴더명으로 쓸 수 없는 문자 제거
        safe_category = re.sub(r'[\\/*?:"<>|]', "", target_category).strip()

        if not safe_category:
            return False
        return self.move_file_safely(file_id, os.path.join(base_dir, safe_category)).get("success", False)
