...
1. Set "@TYPE" to "@검색" IF:
   ...
   - Search terms may match filename, display name, tags, category, or AI summary.
...
"""
        ...
        try:
            ...
            if parsed_json["@TYPE"] == "@검색":
                ...
                # 사용자가 명시한 상대 날짜는 모델 추측보다 규칙 기반 해석을 우선한다.
            ...
            # Ollama가 잠시 꺼져도 명백한 파일 검색은 DB에서 계속 가능하도록 한다.
            return {
                "status": "SUCCESS",
    ...
    def _fallback_search(self, user_text: str) -> Dict[str, Any]:
        """AI 실패 시에도 확장자와 단어로 안전한 로컬 DB 검색을 수행합니다."""
    ...
    def _extract_date_range(text: str) -> Dict[str, str] | None:
        """'지난주', '어제', 날짜 표기를 DB의 파일 수정일 범위로 변환합니다."""
        today = date.today()
...
if __name__ == "__main__":
    parser = SearchQueryParser(model=TEXT_MODEL)
