...
from PySide6.QtCore import QProcess, QUrl, Qt, QThread, Signal
...
class SearchView(QWidget):
    ...
    def init_ui(self):
        self.setStyleSheet("""
            ...
            QPushButton.openLocationBtn:hover { background-color: #F0EEFF; }
    ...
    def _on_file_analyzed(self, summary):
        self.add_message(f"파일 분석 완료: 성공 {summary.get('success', 0)}개, 실패 {len(summary.get('failed', []))}개", is_user=False)
    ...
    def add_result_message(self, text: str, file_path: str):
        """검색 결과에 실제 파일이 있는 탐색기를 여는 동작을 함께 표시합니다."""
        ...
        open_button = QPushButton("탐색기에서 보기")
        open_button.setProperty("class", "openLocationBtn")
        open_button.clicked.connect(lambda _checked=False, path=file_path: self.open_file_location(path))

        result_layout = QVBoxLayout()
        result_layout.setSpacing(6)
        result_layout.addWidget(bubble)
        result_layout.addWidget(open_button, alignment=Qt.AlignLeft)
    ...
    def open_file_location(self, file_path: str):
        """Windows에서는 해당 파일을 선택한 탐색기를, 그 외에는 상위 폴더를 엽니다."""
        if not file_path:
            self.add_message("파일 경로 정보가 없습니다.", is_user=False, kind="error")
            return
        native_path = os.path.normpath(file_path)
        target_dir = native_path if os.path.isdir(native_path) else os.path.dirname(native_path)
        if not os.path.isdir(target_dir):
            self.add_message(f"경로를 찾을 수 없습니다: {target_dir}", is_user=False, kind="error")
            return
        if os.name == "nt" and os.path.isfile(native_path):
            QProcess.startDetached("explorer.exe", [f"/select,{native_path}"])
        else:
